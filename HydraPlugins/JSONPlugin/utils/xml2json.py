...
def elem_to_internal(elem, strip_ns=1, strip=1):
    ...
    for subelem in elem:
        ...
        try:
            ...
            if subelem.attrib.get('type') == 'list':
                d[tag] = [value]
            else:
                d[tag] = value
    text = elem.text
    ...
    if elem.attrib.get('type') is not None:
        if elem.attrib['type'] == 'xs:integer':
            text = int(text)
...
def internal_to_elem(pfsh, factory=ET.Element):
    ...
    if isinstance(value, dict):
        ...
        if type(value) == int:
            attribs['type']='xs:integer'
        else:
            attribs['type']='xs:string'
        text = str(value) # To make incompatible types work.
...
def elem2json(elem, options=None, strip_ns=1, strip=1):
    ...
    if options is not None and options.pretty is True:
...
def xml2json(xmlstring, options=None, strip_ns=1, strip=1):

    """Convert an XML string into a JSON string."""

    elem = ET.fromstring(xmlstring)
    return elem2json(elem, options=None, strip_ns=strip_ns, strip=strip)
