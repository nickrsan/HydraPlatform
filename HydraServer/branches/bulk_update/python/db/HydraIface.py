...
def bulk_insert(objs, table_name):
    ...
    for obj in objs:
        ...
        val = [getattr(obj.db, n) for n in obj.db.db_attrs]
...
def bulk_insert_descriptor(descriptors):
    """
        Insert multiple descriptors, scalars, arrays, timeseries or eqtimeseries at the same time.
    """
    ...
    rs = execute(sql)
    max_data_id = rs[0].max_data_id
    if max_data_id is None:
        max_data_id = 0

    bulk_insert(descriptors)
    ...
    for r in rs:
        sd_i = ScenarioData()
        sd_i.db.data_id = r.data_id
        sd_i.db.data_type = 'descriptor'
