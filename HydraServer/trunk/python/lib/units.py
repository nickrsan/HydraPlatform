...
def add_dimension(user_id, dimension):
...
def delete_dimension(user_id, dimension):
...
def update_unit(user_id, unitdict):
...
def delete_unit(user_id, unitdict):
...
def convert_units(user_id, values, unit1, unit2):
...
def convert_dataset(user_id, dataset_id, to_unit):
...
def get_dimension(user_id, unit1):
...
def get_units(user_id, dimension):
...
def check_consistency(user_id, unit, dimension):
